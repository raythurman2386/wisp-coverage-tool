...
class Antenna:
    ...
    def __post_init__(self):
        ...
        logger.info(f"Successfully created antenna {self.name} at ({self.latitude}, {self.longitude})")
        if self.beam_width is not None:
            logger.info(f"Antenna {self.name} configured with {self.beam_width}° beam width, azimuth: {self.azimuth}°")
